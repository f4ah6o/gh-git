---
name: gh-git
description: "gh-git のリポジトリ単位 GitHub アカウント紐付け、Git author、認証、shell hook、HTTPS/SSH の使い方を説明・診断する。ユーザーが gh git の使い方、bind、unbind、status、accounts、doctor、env、shell-init、credential、または gh git -h のヘルプを尋ねたときに使う。"
---
...
`gh-git` は、リポジトリごとに GitHub アカウントを選択する `gh` 拡張機能である。
...
`credential` は Git の repository-local credential helper から呼ばれる内部コマンドであり、通常は直接実行しない。
...
1. `gh git status` で対象アカウント、host、author、Git HTTPS/SSH 配線を確認する。
2. `gh git status --json` で機械的に結果を確認する。
